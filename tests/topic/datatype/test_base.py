...
def test_kafka_data_type_autoregister(mocker: MockFixture):
    register_schema = mocker.patch.object(
        KafkaDataType, "register_schema", autospec=True
    )
...
def test_kafka_data_type_registering(mocker: MockFixture):
    register = mocker.MagicMock()
    register.register_schema.return_value = 10
    mocker.patch.object(
    ...
    data_type = KafkaDataType(
        SchematicSerializable, topic="test-topic", schema_registry=register
    )
    register.register_schema.assert_called_once_with("subject", "schema string")
...
def test_deserialize_schema_with_invalid_magic_byte(mocker: MockFixture):
    ...
    with pytest.raises(TypeError):
        data_type._deserialize_schema(io.BytesIO(b"\x01\x02\x03\x04\x05"), SchematicSerializable)
...
def test_deserialize_schema(mocker: MockFixture):
    ...
    data_type = KafkaDataType(SchematicSerializable, 'test-topic', register, auto_register_schema=False)
